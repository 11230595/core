...
public class RedisClusterPool {
	 ...
	 * @return
	 */
	public static JedisCluster getJcByPool(){
	...
	private static void gcPool(){
		System.out.println("当前连接池的数量：" + freeJcs.size());
		if(freeJcs.size() > minConn){
			System.out.println("删除多余的连接池");
			try {
	...
	private static void timeGC(){
		...
		timer.schedule(task, 3000); //每6个小时执行一次 60 * 1000 * 60 * 6
	}
}
